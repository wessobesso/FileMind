...
struct ContentView: View {
    ...
    private var mainContentView: some View {
        VStack(spacing: 16) {
            if !uploadedFiles.isEmpty { uploadedFilesView }
        ...
        ZStack(alignment: .topTrailing) {
            ...
            Button(action: {
                if let index = uploadedFiles.firstIndex(of: file) {
                    let removedFile = uploadedFiles.remove(at: index)
    ...
    private var deepSearchToggle: some View {
        VStack(spacing: 6) {
            ...
            if isDeepSearchEnabled {
                    HStack(spacing: 6) {
                        Text("⚠️ Note that DeepSearch is very RAM consuming ⚠️")
                            .font(.subheadline)
                ...
                if isFindModeActive {
                    HStack(spacing: 4) {
                        ...
                        .onChange(of: viewModel.searchText) { _, newText in
                            ...
                            if newText.trimmingCharacters(in: .whitespacesAndNewlines).count > 8 {
                                fetchAIFileMatches(for: newText)
                            }
                        }
                        .onSubmit {
                            if isFindModeActive && viewModel.isFindCommandActive {
                                fetchAIFileMatches(for: viewModel.searchText)
                            }
                        }
...
//MARK: AI File search
    ...
    func fetchAIFileMatches(for prompt: String) {
        aiSearchResults = []  // Clear old results
        print("🤖 Raw input prompt: \(prompt)")

        let cleanedPrompt = prompt
            .replacingOccurrences(of: "/find prompt", with: "", options: .caseInsensitive)
            .replacingOccurrences(of: "/find", with: "", options: .caseInsensitive)
            .trimmingCharacters(in: .whitespacesAndNewlines)

        print("🧠 Sending to OpenAI: \(cleanedPrompt)")

        FileAnalyzer.analyzeFiles(urls: uploadedFiles) { summaries in
            print("📂 Got summaries for \(summaries.count) files")

            let fileDescriptions = summaries.map { url, summary in
                "- \(url.lastPathComponent): \(summary.prefix(300))"
            }.joined(separator: "\n")

            let fullPrompt = """
            You are a file-matching assistant. A user wants to find a file based on this prompt:
            \"\"\"
            \(cleanedPrompt)
            \"\"\"
            Below is a list of files with their content summaries:
            \(fileDescriptions)

            Based on the user’s prompt, return a comma-separated list of filenames that best match. Be selective.
            """

            AIFileSearcher.shared.searchFiles(with: fullPrompt) { aiResponse in
                guard let aiResponse = aiResponse else {
                    print("❌ Failed to get AI response")
                    return
                }

                print("✅ AI selected files: \(aiResponse)")

                let filenames = aiResponse
                    .components(separatedBy: ",")
                    .map { $0.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() }

                let matched = uploadedFiles.filter { file in
                    filenames.contains(file.lastPathComponent.lowercased())
                }

                DispatchQueue.main.async {
                    aiSearchResults = matched.map {
                        FileSearchResult(url: $0, isDirectory: false)
                    }
                }
            }
        }
    ...
    var settingsView: some View {
        Form {
            Section(header: Text("General")) {
                Toggle("Auto-start at login", isOn: .constant(false))
                Toggle("Enable advanced AI analysis", isOn: .constant(true))
            }
