...
class ContentViewModel: ObservableObject {
    ...
    let openAIKey = "na na na boo boo no api key for you"
    ...
    func performFindSearch() {
        let prompt = searchText.replacingOccurrences(of: "/find", with: "").trimmingCharacters(in: .whitespacesAndNewlines)

        guard !prompt.isEmpty else {
            print("❌ Empty prompt provided to /find")
            return
        }
        ...
        let requestBody: [String: Any] = [
            "model": "gpt-4",
            "messages": [
                ["role": "user", "content": "Analyze the following prompt and tell me what the user wants to find. Be concise and clear.\n\n\"\(prompt)\""]
            ],
            "temperature": 0.2
        ]
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            if let error = error {
                print("❌ ChatGPT API call failed: \(error)")
                return
            }
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               ...
               let content = message["content"] as? String {
                print("🤖 User wants: \(content)")
            } else {
                print("❌ Unexpected ChatGPT API response")
            }
        }.resume()
    }
}
